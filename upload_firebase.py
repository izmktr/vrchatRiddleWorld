...
"""
Firebaseアップローダー
raw_dataフォルダの生データをFirebaseにアップロード
"""
...
def main():
    ...
    if not firebase_manager.is_connected():
        print("❌ Firebaseに接続できません")
        print("💡 Firebase設定ファイルを確認してください")
        return
    ...
    try:
        ...
        for i, filename in enumerate(raw_files, 1):
            ...
            try:
                ...
                # Firebase保存
                if firebase_manager.save_world_data(world_data):
        ...
        # 最終統計
        final_stats = firebase_manager.get_stats()
        print(f"\n📈 最終データベース統計:")
